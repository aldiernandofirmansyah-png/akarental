    ...
    $barangs = [
        (object) ['id' => 1, 'nama_barang' => 'Kamera Canon EOS 200D', 'kategori' => 'Kamera', 'harga_sewa' => 150000, 'stok' => 3, 'deskripsi' => 'Kamera DSLR entry level', 'foto' => 'canon.jpg'],
        (object) ['id' => 2, 'nama_barang' => 'Kamera Sony A6400', 'kategori' => 'Kamera', 'harga_sewa' => 200000, 'stok' => 2, 'deskripsi' => 'Kamera mirrorless autofocus cepat', 'foto' => 'sony.jpg'],
        (object) ['id' => 3, 'nama_barang' => 'Tenda 2 Orang', 'kategori' => 'Alat Camping', 'harga_sewa' => 75000, 'stok' => 5, 'deskripsi' => 'Tenda kapasitas 4 orang anti air', 'foto' => 'tenda.jpg'],
        (object) ['id' => 4, 'nama_barang' => 'kompor', 'kategori' => 'Alat Camping', 'harga_sewa' => 35000, 'stok' => 10, 'deskripsi' => 'Sleeping bag nyaman untuk suhu dingin', 'foto' => 'kompor.jpg'],
        (object) ['id' => 5, 'nama_barang' => 'Paket 1', 'kategori' => 'Paket', 'harga_sewa' => 350000, 'stok' => 2, 'deskripsi' => 'Paket hemat: Tenda + Sleeping Bag + Kompor + kursi', 'foto' => 'paket.jpg'],
        (object) ['id' => 6, 'nama_barang' => 'Paket 2', 'kategori' => 'Paket', 'harga_sewa' => 200000, 'stok' => 3, 'deskripsi' => 'Paket hemat: Tenda + Sleeping Bag + Kompor', 'foto' => 'paket.jpg']
    ];
    ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl p-4 text-white shadow-lg">
            <div class="flex justify-between"><div><p class="text-sm opacity-90">Total Barang</p><p class="text-3xl font-bold">{{ $totalBarang }}</p></div><i class="fas fa-box text-3xl opacity-50"></i></div>
        </div>
        <div class="bg-gradient-to-r from-cyan-500 to-cyan-600 rounded-xl p-4 text-white shadow-lg">
            <div class="flex justify-between"><div><p class="text-sm opacity-90">Kategori Kamera</p><p class="text-3xl font-bold">{{ $totalKamera }}</p></div><i class="fas fa-camera text-3xl opacity-50"></i></div>
        </div>
        <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-xl p-4 text-white shadow-lg">
            <div class="flex justify-between"><div><p class="text-sm opacity-90">Kategori Camping</p><p class="text-3xl font-bold">{{ $totalCamping }}</p></div><i class="fas fa-campground text-3xl opacity-50"></i></div>
        </div>
        <div class="bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl p-4 text-white shadow-lg">
            <div class="flex justify-between"><div><p class="text-sm opacity-90">Kategori Paket</p><p class="text-3xl font-bold">{{ $totalPaket }}</p></div><i class="fas fa-gift text-3xl opacity-50"></i></div>
        </div>
        <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-xl p-4 text-white shadow-lg">
            <div class="flex justify-between"><div><p class="text-sm opacity-90">Total Pelanggan</p><p class="text-3xl font-bold">{{ $totalPelanggan }}</p></div><i class="fas fa-users text-3xl opacity-50"></i></div>
        </div>
    </div>

    {{-- TABEL BARANG --}}
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="p-4 border-b bg-gray-50"><h2 class="text-lg font-bold"><i class="fas fa-list text-purple-600 mr-2"></i>Daftar Barang</h2></div>
            ...
            <table class="w-full">
                ...
                <tbody>
                    ...
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3"><div class="w-10 h-10 bg-gray-200 rounded flex items-center justify-center"><i class="fas fa-camera text-gray-400"></i></div></td>
                        ...
                        <td class="px-4 py-3"><span class="px-2 py-1 rounded-full text-xs {{ $barang->stok>0?'bg-green-100 text-green-700':'bg-red-100 text-red-700' }}">{{ $barang->stok>0?'Stok: '.$barang->stok:'Habis' }}</span></td>
                        <td class="px-4 py-3">
    ...
    {{-- ==================== MODAL TAMBAH BARANG (KATEGORI: Kamera, Alat Camping, Paket) ==================== --}}
            ...
            <form onsubmit="event.preventDefault(); alert('Barang berhasil ditambahkan!'); closeModal('tambahBarangModal');">
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <input type="text" placeholder="Contoh: Kamera Canon EOS 200D" class="w-full border rounded-lg px-3 py-2" required>
                    </div>
                    ...
                    {{-- Kategori (Kamera, Alat Camping, Paket) --}}
                    <div>
                    ...
                    <div>
                        ...
                        <textarea rows="3" placeholder="Deskripsi barang..." class="w-full border rounded-lg px-3 py-2"></textarea>
                    </div>
                    
                    {{-- Harga Sewa per Hari --}}
                    <div>
                        <label class="block text-sm font-semibold mb-1">Harga Sewa per Hari</label>
                        <input type="number" placeholder="150000" class="w-full border rounded-lg px-3 py-2" required>
                    </div>
                    
                    {{-- Stok --}}
                    <div>
                        <label class="block text-sm font-semibold mb-1">Stok</label>
                        <input type="number" placeholder="5" class="w-full border rounded-lg px-3 py-2" required>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onsubmit="event.preventDefault(); alert('Barang berhasil diupdate!'); closeModal('editBarangModal');">
                <div class="space-y-3">
                    <div>
                        <label class="block text-sm font-semibold mb-1">Nama Barang</label>
                        <input type="text" id="editNamaBarang" class="w-full border rounded-lg px-3 py-2" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">Foto Barang</label>
                        <input type="file" class="w-full border rounded-lg px-3 py-2">
                    ...
                    <div>
                        <label class="block text-sm font-semibold mb-1">Kategori</label>
                        <select id="editKategori" class="w-full border rounded-lg px-3 py-2" required>
                            <option value="Kamera">📷 Kamera</option>
                            <option value="Alat Camping">⛺ Alat Camping</option>
                            <option value="Paket">📦 Paket</option>
                        </select>
                    ...
                    <div>
                        <label class="block text-sm font-semibold mb-1">Deskripsi</label>
                        <textarea id="editDeskripsi" rows="3" class="w-full border rounded-lg px-3 py-2"></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">Harga Sewa per Hari</label>
                        <input type="number" id="editHarga" class="w-full border rounded-lg px-3 py-2" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold mb-1">Stok</label>
                        <input type="number" id="editStok" class="w-full border rounded-lg px-3 py-2" required>
                    </div>
                </div>
                <div class="flex gap-3 mt-6">
